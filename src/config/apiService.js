...
import { getAxiosApiConfig, spinLoaderConfigDefault } from './apiConfig';
...
class ApiService {
  constructor(config, apiKeyParam) {
    const isValidObject = input => input && typeof input === 'object' && Object.keys(input).length;
    ...
    if (apiKey) {
      ...
      if (isValidObject(this.config.spinLoaderConfig)) {
        if (this.config.spinLoaderConfig.isEnabled) {
          ...
          if (typeof this.apiConfig.spinLoaderConfig.actionFunction !== 'function') {
            this.apiConfig.spinLoaderConfig.actionFunction = spinLoaderConfigDefault.actionFunction;
          }
        }
      } else {
